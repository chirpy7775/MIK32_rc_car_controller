...
void SystemClock_Config(void)
{
    ...
    p.AHBDivider = p.APBMDivider = p.APBPDivider = 0;
...
static void Timer32_1_Init(void)
{
    ...
    htimer32_1.Clock.Prescaler = 31;          // 32 МГц / 32 / 2 kГц (20 000 мкс)
...
int main(void)
{
    ...
    while (1) {
        ...
        if (!UART_IsRxFifoEmpty(UART_1)) {
            uint8_t c = UART_ReadByte(UART_1);
            if (c == '\r' || c == '\n') {
                if (idx) {
                    switch (uart_buf[0]) {
                        case 's':   /* servo angle */
                            cmd = simple_atoi(uart_buf + 1, idx - 1);
                            Servo_SetAngle(cmd);
                            break;
                        case 't':   /* throttle forward */
                            cmd = simple_atoi(uart_buf + 1, idx - 1);
                            ESC_SetThrottle((int16_t)cmd);
                            break;
                        case 'b':   /* throttle backward */
                            ...
                            break;
                        case 'm':   /* SERVO_MIN */
                            cfg.servo_min_us = simple_atoi(uart_buf + 1, idx - 1);
                            xprintf("m set\n");
                            break;
                        case 'M':   /* SERVO_MAX */
                            cfg.servo_max_us = simple_atoi(uart_buf + 1, idx - 1);
                            xprintf("M set\n");
                            break;
                        case 'e':   /* echo threshold */
                            cfg.echo_threshold_us = simple_atoi(uart_buf + 1, idx - 1);
                            xprintf("e set\n");
                            break;
                        case 'c':   /* command timeout */
                            cfg.command_timeout_ms = simple_atoi(uart_buf + 1, idx - 1);
                            xprintf("c set\n");
                            break;
                    }
